...
The 2026-03-23 **Unread Activity + Needs Patrick Indicators**, **Card-Level Model Selector**, and **Agent Question Reply-to-Resume** features do **not** require new deployment-specific environment variables.
...
- the existing Mission Control server auth/token model
...
### Reply-to-resume deployment note

The reply-to-resume flow uses **internally injected** callback env vars for each spawned agent run:
- `MC_CARD_API_URL`
- `MC_AUTH_TOKEN`

These are generated by Mission Control itself at runtime. Operators do **not** need to provide them in Northflank or wrapper-server configuration.

The only deployment assumptions are:
- Mission Control can still spawn `openclaw agent ...` child processes
- spawned runs inherit the Mission Control process environment normally
- Mission Control's own auth/token state file under `/data/.gstack/` remains writable
